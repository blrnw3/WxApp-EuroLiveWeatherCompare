...
while($row = mysql_fetch_assoc($result)) {
	...
	if($cityName == 'Hampstead') { // Special case for my weather station in Hampstead, North London
		...
		$url = "http://api.worldweatheronline.com/free/v1/weather.ashx?key=".$APIkey."&query=".$APIquery."&format=xml&includeLocation=yes";
		$xml = getXml($url);
...
function get_nw3_wxicon() {
	$METAR = file_get_contents("/home/nwweathe/public_html/METAR.txt");
	echo ($METAR), '<br />';
